...
评测规模：政策 709 + 工具 16 + 超纲拒答 10 + 敏感审批 6 = 741 题。
注：retrieval 评测覆盖全部政策问题（含扩展集）；端到端评测排除会触发敏感审批的证明类问题，
    因此端到端题目数较多、需要真实 LLM 调用，建议按需运行。
...
2. 版本管理：任何题目增删改都必须递增 DATASET_VERSION，评测报告会记录该版本；
   切分策略变更（如 2026.09-v4 引入表格结构化提取）同样递增版本并重录基线，
   避免跨切分布局比较指标；调参阶段只允许看开发集，禁止反复在同一套题上刷指标后当作
   泛化结果汇报。
...
# v4（2026-09-15）：知识库引入 Markdown 表格结构化提取（agent/chunking.py），
# chunk 布局变化，按规范递增版本并重录检索基线。
DATASET_VERSION = "2026.09-v4"
