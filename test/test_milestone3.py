...
def chat_with_agent(uid: str, question: str):
    ...
    for event in hr_agent_app.stream(initial_state, stream_mode='values'):
        ...
        # 过滤掉用户输入保持日志干净；审计打回提示出现时额外打印一条提示
        # 注意：打回消息前缀须与 graph_builder.py 中注入的 SYSTEM AUTO FAILED 保持一致
        if isinstance(last_msg, HumanMessage):
            if 'SYSTEM AUTO FAILED' in last_msg.content:
                print(f'\n [审计打回，要求重写] {last_msg.content[:80]}…')
            continue
